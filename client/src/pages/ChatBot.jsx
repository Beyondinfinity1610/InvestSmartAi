...
const ChatBot = ({ isDarkMode, selectedInvestment, portfolioData }) => {
  ...
  useEffect(() => {
    if (selectedInvestment) {
      setMessages([
        {
          role: "assistant",
          content: `I'm analyzing ${selectedInvestment.name} (${selectedInvestment.symbol}). 
  ...
  const generateAIResponse = (message) => {
    ...
    if (!selectedInvestment) {
      return `Please select an investment from the left panel for detailed analysis. For your overall portfolio: 
        - Total value: $${portfolioData?.totalAssets?.toLocaleString() || "0"}
        - Performance: ${portfolioData?.performance || "N/A"}
        - Monthly growth: ${portfolioData?.monthlyGrowth || "0"}%`;
    }
  ...
  const sendMessageToAI = async (message) => {
    try {
      ...
      let context = "";
      if (selectedInvestment) {
        context = `Selected Investment: ${selectedInvestment.name} (${selectedInvestment.symbol})
          Price: $${selectedInvestment.price}
          Allocation: ${selectedInvestment.allocation}%
          Value: $${selectedInvestment.value}`;
      } else if (portfolioData) {
        context = `Portfolio Overview:
          Total Assets: $${portfolioData.totalAssets?.toLocaleString() || "0"}
          Performance: ${portfolioData.performance || "N/A"}
          Monthly Growth: ${portfolioData.monthlyGrowth || "0"}%`;
      }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const mockResponse = generateAIResponse(userMessage);
      setMessages((prev) => [
        ...prev,
  ...
  return (
    <div className={`h-full flex flex-col ${isDarkMode ? "bg-gray-900" : "bg-white"}`}>
      <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <button className={`p-1 rounded-full ${isDarkMode ? "hover:bg-gray-800" : "hover:bg-gray-100"}`}>
          ✕
        </button>
      </div>
